...
class DuelingDQN(nn.Module):
    ...
    def __init__(self, n_angle, n_actions, n_hidden1, n_hidden2):
        ...
        self.fc1 = nn.Linear(self.hidden2//2, 8)
        # self.fc2 = nn.Linear(, 64)
        self.valuelayer = nn.Linear(8,3)
        self.actionlayer = nn.Linear(8,1)
    ...
    def forward(self, state):
        ...
        # x1 = x1.unsqueeze(0)
        # x2 = x2.unsqueeze(0)
        self.value = F.relu(self.valuelayer(F.relu((self.fc1(x1)))))
        self.action = F.relu(self.actionlayer(F.relu(self.fc1(x1))))
